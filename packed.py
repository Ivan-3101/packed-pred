...
FLASK_API_URL = "https://packed-api.onrender.com/analyze_ingredients"  # URL of your Flask API
...
def call_flask_api(ingredients_text):
    """Call the Flask API to analyze ingredients."""
    response = requests.post(
        FLASK_API_URL,
        json={"ingredients": ingredients_text}
    )
    
    if response.status_code == 200:
        data = response.json()
        return data["analysis"], data["healthy_count"], data["concerning_count"]
    else:
            ...
            if st.button("🔍 Analyze Ingredients", key="analyze_button"):
                ...
                if ingredients_text:
                    ...
                    with st.spinner("Analyzing ingredients..."):
                        analysis, healthy_count, concerning_count = call_flask_api(ingredients_text)
